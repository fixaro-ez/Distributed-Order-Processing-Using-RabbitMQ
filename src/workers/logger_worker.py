...
def main():
    ...
    def handle_message(ch, method, properties, body):
        ...
        print(
            "[logger] Ticket -> "
            f"Order {payload['order_id']} | {payload['quantity']} x {payload['item']} | "
            f"Email {payload['email']} | Notes: {payload['notes'] or 'None'}"
        )
        ch.basic_ack(delivery_tag=method.delivery_tag)

    channel.basic_qos(prefetch_count=1)
    channel.basic_consume(queue="logger_queue", on_message_callback=handle_message)

    print("[logger] Waiting for messages. Press CTRL+C to exit.")
    try:
